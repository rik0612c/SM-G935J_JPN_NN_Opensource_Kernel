...
static void mdss_samsung_event_frame_update(struct mdss_panel_data *pdata, int event, void *arg)
{
	...
	if (vdd->multires_stat.is_support && ctrl->ndx == DISPLAY_1) {
		if (vdd->multires_stat.black_frame_cnt) {
			vdd->multires_stat.black_frame_cnt--;
			if(!vdd->multires_stat.black_frame_cnt)
				mdss_samsung_send_cmd(ctrl, PANEL_DISPLAY_ON);
		}
	}
	...
	u32 *flags)
{
	...
	switch (cmd) {
		...
		case PANEL_MDNIE_TUNE:
			if (vdd->support_hall_ic) //temporary, MUST FIX
				cmds = &vdd->mdnie_tune_data[DISPLAY_1].mdnie_tune_packet_tx_cmds_dsi;
			else
		...
		case PANEL_MULTIRES_HD:
			...
			break;
		case PANEL_PPS_WQHD:
			LCD_INFO("PANEL_PPS_WQHD\n");
			cmds = &vdd->dtsi_data[ndx].panel_pps_wqhd[vdd->panel_revision];
			break;
		case PANEL_PPS_FHD:
			LCD_INFO("PANEL_PPS_FHD\n");
			cmds = &vdd->dtsi_data[ndx].panel_pps_fhd[vdd->panel_revision];
			break;
		case PANEL_PPS_HD:
			LCD_INFO("PANEL_PPS_HD\n");
			cmds = &vdd->dtsi_data[ndx].panel_pps_hd[vdd->panel_revision];
			break;
...
{
	LCD_ERR("lcd esd recovery\n");
	/* esd status must return 0 to go status_dead(blank->unblnk)
	 * in mdss_check_dsi_ctrl_status.
	 */
	return 0;
}
...
{
	LCD_ERR("lcd esd - check_read_status\n");
	/* esd status must return 0 to go status_dead(blank->unblnk)
	 * in mdss_check_dsi_ctrl_status.
	 */
	return 0;
			...
			struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	for (panel_rev = 0; panel_rev < SUPPORT_PANEL_REVISION; panel_rev++) {
		...
		parse_dt_data(np, &dtsi_data->smart_acl_elvss_lowtemp2_tx_cmds[panel_rev],
		...
		/* DSC PPS */
		parse_dt_data(np, &dtsi_data->panel_pps_wqhd[panel_rev],
				"samsung,panel_pps_cmd_wqhd_rev", panel_rev, NULL);
		parse_dt_data(np, &dtsi_data->panel_pps_fhd[panel_rev],
				"samsung,panel_pps_cmd_fhd_rev", panel_rev, NULL);
		parse_dt_data(np, &dtsi_data->panel_pps_hd[panel_rev],
				"samsung,panel_pps_cmd_hd_rev", panel_rev, NULL);
...
static ssize_t mdss_samsung_current_resolution_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int mode = 0;
	struct samsung_display_driver_data *vdd =
	...
	int rc = 0;
	sec_get_param(param_index_lcd_resolution, vdd->resolution_sec_param);

	if(vdd->resolution_sec_param[0] == 0)
	{
		LCD_INFO("force to %s\n", vdd->resolution_default);
		memcpy(vdd->resolution_sec_param, vdd->resolution_default, sizeof(vdd->resolution_default));
	}
	LCD_INFO("vdd_data->resolution_from_sec_param = %s\n", vdd->resolution_sec_param);

	if (!strcmp(vdd->resolution_sec_param,"WQHD"))
		mode = MULTIRES_WQHD;
	else if (!strcmp(vdd->resolution_sec_param,"FHD"))
		mode = MULTIRES_FHD;
	else if (!strcmp(vdd->resolution_sec_param,"HD"))
		mode = MULTIRES_HD;

	if (vdd->panel_func.samsung_pps_setting)
		vdd->panel_func.samsung_pps_setting(vdd, mode);

	rc = snprintf((char *)buf, sizeof(vdd->resolution_sec_param), "%s\n", vdd->resolution_sec_param);
	return rc;
}
...
static DEVICE_ATTR(resolution, S_IRUGO | S_IWUSR | S_IWGRP, mdss_samsung_current_resolution_show, NULL);
...
static void dump_reg(char *addr, int len)
{
	...
	mdss_dump_reg("ss_dump", MDSS_DBG_DUMP_IN_LOG, addr, len, NULL);
